...
Emax = -5.0             # Maximum energy
Emin = +5.0             # Minimum energy
...
for trial in range(1,(ntrials+nequil+1)):
    ...
    if trial > nequil:
        energy+=2*deltaE/n2
        if energy > Emax:   #check if energy has reached a new max
            Emax = energy
        if energy < Emin:   #check if energy has reached a new min
            Emin = energy
        E_sum+=energy
...
print('%10.6f %10.6f'%(Emin, Emax)) #print min and max energy values
